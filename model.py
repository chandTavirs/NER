...
def get_lstm_features(self, sentence, chars2, chars2_length, d):
    ...
    if self.word_mode == 'CNN':
        words_cnn_out3 = self.word_cnn3(embeds)

        # cnn_out = nn.functional.max_pool2d(words_cnn_out3, kernel_size=(1, words_cnn_out3.size(3))).squeeze(-1).squeeze(-1)

        cnn_out = words_cnn_out3.view(len(sentence), self.hidden_dim * 2)
...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim,
                 ...
                 dropout=0.5, use_gpu=True, use_crf=True, char_mode='CNN', word_mode='LSTM',
                 num_word_cnn_layers=1, dilation=False):
        '''
        Input parameters:
        ...
        self.num_word_cnn_layers = num_word_cnn_layers
        self.combined_embed_dim = embedding_dim +char_out_dimension
        ...
        if char_embedding_dim is not None:
            ...
            if self.char_mode == 'LSTM':
                ...
                self.combined_embed_dim = self.embedding_dim + 2* self.char_lstm_dim
            ...
            if self.char_mode == 'CNN':
                ...
                self.combined_embed_dim = self.embedding_dim + self.out_channels
        ...
        if self.word_mode == 'CNN':
            if self.num_word_cnn_layers == 1:
                self.word_cnn3 = nn.Sequential(
                    nn.Conv2d(in_channels=1, out_channels=hidden_dim * 2, kernel_size=(3, 3), padding=(1, 1)),
                    nn.MaxPool2d((1, self.combined_embed_dim)))


            elif self.num_word_cnn_layers == 2:
                self.word_cnn3 = nn.Sequential(
                    nn.Conv2d(in_channels=1, out_channels=hidden_dim * 2, kernel_size=(3, 3), padding=(1, 1)),
                    nn.MaxPool2d((1, 2)),
                    nn.Conv2d(in_channels=hidden_dim * 2, out_channels=hidden_dim * 2, kernel_size=(3, 3),
                              padding=(1, 1)),
                    nn.MaxPool2d((1, self.combined_embed_dim // 2)))

            elif self.num_word_cnn_layers == 3:
